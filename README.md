...
- 分类标签：后台 CRUD、前台公开列表、文章筛选；公开分类、标签和专栏的文章数量只统计公开已发布文章。
...
- 站点设置：后台站点信息维护、前台布局和关于页读取；公开设置接口不返回记录 ID 和创建/更新时间。
...
- 学习路线与题库入口：前台新增题库页面，集中展示计算机基础、Java 后端、数据库、前端、工程工具、系统设计、AI 与安全等方向，并提供面试题训练入口和相关文章跳转。
- 内容发现：公开侧栏热门文章排行、文章详情相关文章推荐、后台数据概览热门公开文章排行。
- 文章互动：匿名访客幂等点赞/取消点赞、点赞总数、两层评论回复、后台评论/留言/友链申请审核；公开评论、留言、友链申请和友链列表不返回访客邮箱、审核状态、后台显示状态等内部字段。
